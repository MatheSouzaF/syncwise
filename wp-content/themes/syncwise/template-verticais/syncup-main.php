...
<div class="tabs-content lte sync" id="tab-2">

    <div class="wrapper">
        <div class="box-content">
            <p class="label-synciq"><?php echo get_field('title_lte_global_sim'); ?></p>
            <div class="box-svg">
                <?php $svg_file = get_field('svg__lte_global_sim');
                if ($svg_file && pathinfo($svg_file['url'], PATHINFO_EXTENSION) === 'svg') {
                    echo '<i class="element">';
                    echo file_get_contents($svg_file['url']);
                    echo '</i>';
            ...
            <p class="description-synciq">
                <?php echo get_field('description_lte_global_sim'); ?>
            </p>
            <div class="box-repeater-item">
                <?php
                if (have_rows('repeater_itens_lte_global_sim')):
                    while (have_rows('repeater_itens_lte_global_sim')):
                        ...
                        <div class="row-item">
                            ...
                            <p class="item-text">
                                <?php echo get_sub_field('text_label_repeater_lte_global_sim'); ?>
                            </p>
                        </div>
                    <?php endwhile;
                endif; ?>
            ...
            <p class="description-synciq">
                <?php echo get_field('description_lte_global_sim_02'); ?>
            </p>
            <?php
            $link = get_field('link_lets_talk_sim');
            if ($link):
                $link_url = $link['url'];
                $link_title = $link['title'];
                $link_target = $link['target'] ? $link['target'] : '_self'; ?>
        ...
        <div class="box-image-video">
            <div class="swiper-container swiper-colecoes">
                <div class="swiper-wrapper">
                    <?php if (have_rows('video_or_image_global')): ?>
                        <?php while (have_rows('video_or_image_global')):
                            the_row(); ?>
                            <?php
                            $slide_index = get_row_index() - 1; // 0-based
                            $extra_class = ''; // inicializa vazio
                    
                            if (get_row_layout() == 'video_global_sim') {
                                $video_modal = get_sub_field('video_modal');
                                if (!empty($video_modal)) {
                                    $extra_class = ' has-modal';
                                }
                            } elseif (get_row_layout() == 'image_global_sim') {
                                $image_has_modal = get_sub_field('video_modal_imagem_global_sim');
                                if (!empty($image_has_modal)) {
                                    $extra_class = ' has-modal';
                                }
                            }
                            ...
                            <div class="swiper-slide <?php echo $extra_class; ?>"
                                data-slide-index="<?php echo esc_attr($slide_index); ?>">
                                <?php if (get_row_layout() == 'video_global_sim'):
                                    $video_url = get_sub_field('video_global_sim_');
                                    $video_modal = get_sub_field('video_modal');
                                    if ($video_url): ?>
                                        <div class="video-wrapper">
                                            <video class="video-banner" autoplay muted loop playsinline
                                ...
                                <?php elseif (get_row_layout() == 'image_global_sim'):
                                    $image = get_sub_field('imagem_global_sim');
                                    $image_has_modal = get_sub_field('video_modal_imagem_global_sim');
                                    if (!empty($image)):
                                        $image_url = $image['url'] ?? '';
                                        $image_alt = $image['alt'] ?? ''; ?>
                ...
                <?php while (have_rows('video_or_image_global')):
                    ...
                    <div class="swiper-slide" data-slide-index="<?php echo esc_attr($slide_index); ?>">
                        ...
                        <?php elseif (get_row_layout() == 'image_global_sim'): ?>
                            ...
                            <div class="box-video-player-about">
                                <video id="player-about">
                                    <source src="<?php echo esc_url(get_sub_field('video_modal_imagem_global_sim')); ?>" type="video/mp4" />
                                </video>
                            </div>
                        <?php endif; ?>
                    </div>
